...
$$
\mathsf{FoundationalClassName}\ =\ \{\texttt{Bitcopy},\ \texttt{Clone},\ \texttt{Drop},\ \texttt{FfiSafe},\ \texttt{Eq},\ \texttt{Hasher},\ \texttt{Hash},\ \texttt{Iterator},\ \texttt{Discrete}\}
$$
...
$$
\begin{array}{l}
\operatorname{BuiltinDiscreteType}(T)\ \Leftrightarrow \ \operatorname{StripPerm}(T)\ =\ \operatorname{TypePrim}(t)\ \land \ t\ \in \ \mathsf{IntTypes}\ \cup \ \mathsf{UnsignedIntTypes}\ \cup \ \{\texttt{char}\} \\[0.16em]
...
\operatorname{ImplementsDiscrete}(T)\ \Leftrightarrow \ \operatorname{BuiltinDiscreteType}(T)\ \lor \ \texttt{Discrete}\ \in \ \operatorname{Implements}(T) \\[0.16em]
...
$$
\texttt{Discrete::successor}\ \mathsf{and}\ \texttt{Discrete::predecessor}\ \mathsf{define}\ a\ \mathsf{discrete}\ \mathsf{stepping}\ \mathsf{relation}\ \mathsf{and}\ \mathsf{are}\ \mathsf{partial}\ \mathsf{inverses}\ \mathsf{when}\ \mathsf{both}\ \mathsf{are}\ \mathsf{defined}.
$$
...
$$
\texttt{Eq::eq}\ \mathsf{on}\ \texttt{EqType(T)}\ \mathsf{lowers}\ \mathsf{intrinsically}\ \mathsf{to}\ \mathsf{the}\ \mathsf{built}-\mathsf{in}\ \mathsf{equality}\ \mathsf{relation}\ \mathsf{for}\ \texttt{T}.\ \texttt{Discrete::successor}\ \mathsf{and}\ \texttt{Discrete::predecessor}\ \mathsf{on}\ \texttt{BuiltinDiscreteType(T)}\ \mathsf{lower}\ \mathsf{intrinsically}\ \mathsf{to}\ \mathsf{the}\ \mathsf{built}-\mathsf{in}\ \mathsf{stepping}\ \mathsf{relation}\ \mathsf{for}\ \texttt{T}.\ \mathsf{Other}\ \texttt{Eq}\ \mathsf{and}\ \texttt{Discrete}\ \mathsf{calls}\ \mathsf{lower}\ \mathsf{through}\ \mathsf{ordinary}\ \mathsf{method}-\mathsf{call}\ \mathsf{lowering}.
$$
